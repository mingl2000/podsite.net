...
- `npm run build` completes with no errors and produces static output
- Public routes generate as static pages (home, episodes list, each episode page, about/contact if present)
- Content files exist and include required metadata: `title`, `date`, `slug`, `description`
- Pages render meaningful HTML without relying on client-only JS (content visible in view-source)
- Responsive: key pages verified at 320px, 768px, 1024px
- Images and assets are optimized or constrained (reasonable file sizes)
- Basic accessibility: images have `alt` text, headings follow a logical order, keyboard navigation works for main flows
- Site artifacts are deployable to a static host (Vercel/Netlify/GitHub Pages)

### Assumptions / Best Guesses Used

- Framework: Next.js with SSG by default
- Content format: Markdown for episodes and JSON/YAML for minor metadata
- Styling: Tailwind or CSS Modules allowed; minimal JS preferred
